...
Result ModbusCodec::parsePdu(uint8_t expected_slave_id,
    ...
    if (pdu.size() < 3) {
        SHOW_ERROR("Modbus",
                  "PDU too short (" + std::to_string(pdu.size()) +
                      " bytes). Expected at least 3 (slave_id + "
                      "func_code + byte_count).");
    ...
    if (func_code != 0x03 && func_code != 0x04) {
        SHOW_ERROR("Modbus", "Unexpected function code in read response: " + toHex(func_code));
